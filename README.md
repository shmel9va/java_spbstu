...
- `mysql` - использует MySQL базу данных через JPA репозитории с миграциями Flyway
   ...
   ---
   # Кэширование с использованием Redis - Step 6
   
   ## Описание
   
   В рамках шага 6 была добавлена система кэширования с использованием Redis для повышения производительности приложения при работе с часто запрашиваемыми данными.
   
   ## Основные изменения:
   
   1. **Настроен Redis для кэширования:**
   - Добавлены зависимости для работы с Redis
   - Настроено подключение к Redis серверу
   - Создан и настроен CacheManager с оптимальными параметрами TTL
   
   2. **Реализовано кэширование основных операций:**
   - Кэширование всех задач пользователя (TTL: 15 минут)
   - Кэширование незавершенных задач (TTL: 5 минут)
   - Кэширование отдельных задач (TTL: 30 минут)
   
   3. **Преимущества использования Redis:**
   ...
   - Распределенное кэширование для масштабируемости
   
   ## Как это работает
   
   1. При запросе данных система сначала проверяет наличие их в кэше Redis
   2. Если данные найдены в кэше и не истек срок их жизни - они возвращаются без обращения к базе данных
   3. Если данные отсутствуют в кэше или истек срок их жизни - выполняется запрос к базе данных, а результаты сохраняются в кэш
   
   ## Конфигурация Redis в Docker
   
   Для запуска приложения с Redis в Docker:
